...
pub async fn chat_completions(
    ...
    info!("Received chat completion request, Content-Type: {}", content_type);
    ...
    let bytes = match axum::body::to_bytes(request.into_body(), usize::MAX).await {
        ...
        Err(e) => {
            ...
            return Err(PluginError::HttpError("Failed to read request body".to_string()));
        }
    };
    ...
    let chat_request: ChatCompletionRequest = match content_type {
        ct if ct.contains("application/json") => {
            match serde_json::from_slice(&bytes) {
                ...
                Err(e) => {
                    ...
                    return Err(PluginError::HttpError("Failed to parse JSON request".to_string()));
                }
            }
        },
        ...
        _ => {
            ...
            return Err(PluginError::HttpError(format!("Unsupported Content-Type: {}", content_type)));
        }
    };
    
    info!("Received chat completion request, model: {}", chat_request.model);
...
pub async fn completions(
    ...
    info!("Received completion request, Content-Type: {}", content_type);
    ...
    info!("Received completion request, model: {}", completion_request.model);
...
pub async fn chat_completions(
    ...
    Err(PluginError::HttpError("http_api feature is not enabled".to_string()))
}
